...
async function init() {
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    try {
      ...
      await pool.query("ALTER TABLE messages ADD CONSTRAINT messages_type_check CHECK(type IN ('text','image','system','voice','video','audio','document'))");
      await pool.query("ALTER TABLE messages ADD COLUMN IF NOT EXISTS edited_at TIMESTAMPTZ");
      await pool.query(`CREATE TABLE IF NOT EXISTS message_deletions (
...
async function getSuggestedUsers(userId) {
  const { rows } = await pool.query(
    `SELECT id, phone, username, display_name, avatar FROM users
...
async function getContacts(userId) {
  const { rows } = await pool.query(
    `SELECT u.id, u.phone, u.username, u.display_name, u.avatar, u.bio, u.last_seen_at
...
async function getUserChats(userId) {
  const { rows } = await pool.query(
    `SELECT c.id, c.type, c.name, c.avatar, c.created_at, c.pinned,
     ...
     (SELECT status FROM messages WHERE chat_id = c.id ORDER BY id DESC FETCH FIRST 1 ROWS ONLY) as last_message_status
...
async function getUserChat(chatId, userId) {
  const { rows } = await pool.query(
    `SELECT c.id, c.type, c.name, c.avatar, c.created_at, c.pinned,
     (SELECT text FROM messages WHERE chat_id = c.id ORDER BY id DESC FETCH FIRST 1 ROWS ONLY) as last_message,
     (SELECT created_at FROM messages WHERE chat_id = c.id ORDER BY id DESC FETCH FIRST 1 ROWS ONLY) as last_message_time,
...
async function getMessages(chatId, limit = 50, cursor = null, userId = 0) {
...
async function searchMessages(chatId, query, limit = 50, userId = 0) {
  const { rows } = await pool.query(
    `SELECT m.*, u.display_name as sender_name, u.avatar as sender_avatar
     FROM messages m JOIN users u ON m.sender_id = u.id
     WHERE m.chat_id = $1 AND m.text ILIKE $2
     AND m.id NOT IN (SELECT message_id FROM message_deletions WHERE user_id = $3)
     ORDER BY m.created_at DESC FETCH FIRST $4 ROWS ONLY`,
    [chatId, `%${query}%`, userId, limit]
  );
  return rows.reverse();
}
...
async function editMessage(messageId, userId, newText) {
  const { rowCount } = await pool.query(
    'UPDATE messages SET text = $1, edited_at = NOW() WHERE id = $2 AND sender_id = $3',
...
async function getRecommendedPosts(userId, cursor = null, limit = 10, seenPostIds = []) {
  ...
  try {
    ...
    if (!candidates.length) {
      // Fallback: return recent posts not seen by the user
      client.release();
      const { rows: fallback } = await pool.query(`
        ...
        JOIN users u ON u.id = p.user_id
        WHERE p.expires_at > NOW()
        AND p.id != ALL($1::bigint[])
        ORDER BY p.created_at DESC
        FETCH FIRST $2 ROWS ONLY
      `, [excludeIds.size ? [...excludeIds] : [0], limit]);
      return fallback;
    }
...
async function deleteChat(chatId, userId) {
  ...
  const { rows } = await pool.query('SELECT COUNT(*) as cnt FROM chat_members WHERE chat_id = $1', [chatId]);
...
async function getUserChatIds(userId) {
  const { rows } = await pool.query('SELECT chat_id FROM chat_members WHERE user_id = $1', [userId]);
...
module.exports = {
  ...
  getUserById, searchUsers, searchPosts, getSuggestedUsers, addContact, getContacts,
  createPrivateChat, getPrivateChat, createGroup, getUserChats, getUserChat, getUserChatIds, getChatMembers, addChatMember,
  addMessage, getMessages, getUnreadCount, markRead, updateMessageStatus, getMessageById, searchMessages, deleteMessage,
  editMessage, deleteForMe, forwardMessage,
  ...
  setOnline, setOffline, isOnline, getOnlineUsers, updateLastSeen,
  updateProfile, close,
