...
| File | Role |
|---|---|
| `backend/lib/recommend.js` | Core algorithm: scoring, MMR, bandit, online learning |
| `backend/db/init.js` | DB integration: `recordInteraction`, `getRecommendedPosts`, `getOrCreateUserEmbedding` |
| `backend/db/007_recommendations.sql` | Schema: post_tags, post_interactions, user_interests, user_embeddings, global_embeddings |
| `backend/server.js` | Socket handlers: `record_interaction`, `get_recommended_posts`, `get_trending_tags` |
| `frontend/src/routes/(app)/for-you/+page.svelte` | TikTok-style feed UI |
| `backend/tests/handlers.test.js` | Unit tests (snowflake, sanitize, healthcheck) |
