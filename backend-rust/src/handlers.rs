...
use crate::protocol::{self, Frame, json_payload, msg_type};
use crate::db;
...
pub async fn register_handlers(
                ...
                Ok(true) => {
                    ...
                    match db::find_or_create_user(&ctx.conn_mgr.db, phone, username, display_name, country_code).await {
                        Ok((user, is_new)) => {
                            ...
                            if let Some(k) = ctx.kafka.clone() {
                                let key = user.id.to_string();
                                let val = rmp_serde::to_vec(&serde_json::json!({"userId": user.id, "online": true})).unwrap();
                                tokio::spawn(async move { k.produce("user-presence", &key, &val).await });
                            }
                        }
                        Err(e) => ctx.respond_json(&frame, &serde_json::json!({"ok": false, "error": e.to_string()})),
                    }
    ...
    registry.register(msg_type::CHAT_SEND, Arc::new(|ctx, frame| {
        Box::new(Box::pin(async move {
            ...
            match db::add_message(&ctx.conn_mgr.db, chat_id, ctx.user.id, &text, content_type, reply_to, None).await {
                Ok(msg_id) => {
                    ...
                    if let Some(k) = ctx.kafka.clone() {
                        ...
                        let val = rmp_serde::to_vec(&serde_json::json!({"chatId": chat_id, "msg": msg, "senderId": ctx.user.id})).unwrap();
                        tokio::spawn(async move { k.produce("chat-messages", &key, &val).await });
                    }
                }
                Err(e) => ctx.respond_json(&frame, &serde_json::json!({"ok": false, "error": e.to_string()})),
            }
    ...
    registry.register(msg_type::CHAT_GET_MESSAGES, Arc::new(|ctx, frame| {
        Box::new(Box::pin(async move {
            ...
            match db::get_messages(&ctx.conn_mgr.db, chat_id, limit, cursor).await {
    ...
    registry.register(msg_type::CHAT_TYPING, Arc::new(|ctx, frame| {
        Box::new(Box::pin(async move {
            ...
            if let Some(k) = ctx.kafka.clone() {
                let key = chat_id.to_string();
                let val = rmp_serde::to_vec(&serde_json::json!({"chatId": chat_id, "frameBase64": fb, "senderId": ctx.user.id, "type": "typing"})).unwrap();
                tokio::spawn(async move { k.produce("chat-events", &key, &val).await });
            }
        }))
    })).await;
    ...
    registry.register(msg_type::CHAT_STOP_TYPING, Arc::new(|ctx, frame| {
        Box::new(Box::pin(async move {
            ...
            if let Some(k) = ctx.kafka.clone() {
                let key = chat_id.to_string();
                let val = rmp_serde::to_vec(&serde_json::json!({"chatId": chat_id, "frameBase64": fb, "senderId": ctx.user.id, "type": "stop_typing"})).unwrap();
                tokio::spawn(async move { k.produce("chat-events", &key, &val).await });
            }
        }))
    })).await;
    ...
    registry.register(msg_type::POST_CREATE, Arc::new(|ctx, frame| {
        Box::new(Box::pin(async move {
            ...
            match db::create_post(&ctx.conn_mgr.db, ctx.user.id, text, media, post_type).await {
    ...
    registry.register(msg_type::POST_GET, Arc::new(|ctx, frame| {
        Box::new(Box::pin(async move {
            ...
            let result = if data.get("type").and_then(|v| v.as_str()) == Some("recommended") {
                let limit = data.get("limit").and_then(|v| v.as_i64()).unwrap_or(20) as i32;
                let seen_ids: Vec<i64> = data.get("seenIds").and_then(|v| v.as_array()).map(|a| a.iter().filter_map(|x| x.as_i64()).collect()).unwrap_or_default();
                db::get_recommended_posts(&ctx.conn_mgr.db, ctx.user.id, limit, seen_ids).await.map(|v| serde_json::to_value(v).unwrap_or_default())
            } else if let Some(uid) = data.get("userId").and_then(|v| v.as_i64()) {
                db::get_user_posts(&ctx.conn_mgr.db, uid).await
            } else {
                let filter = data.get("filter").and_then(|v| v.as_str()).unwrap_or("all");
                let cursor = data.get("cursor").and_then(|v| v.as_str()).map(|s| s.to_string());
                let limit = data.get("limit").and_then(|v| v.as_i64()).unwrap_or(50);
                db::get_posts(&ctx.conn_mgr.db, ctx.user.id, filter, cursor, limit).await.map(|v| serde_json::to_value(v).unwrap_or_default())
            };
            match result {
    ...
    registry.register(msg_type::LIVE_END, Arc::new(|ctx, frame| {
        Box::new(Box::pin(async move {
            ...
            match db::end_live(&ctx.conn_mgr.db, live_id, ctx.user.id).await {
                Ok(_) => ctx.respond_json(&frame, &serde_json::json!({"ok": true})),
                Err(_) => ctx.respond_json(&frame, &serde_json::json!({"ok": false})),
            }
        }))
    ...
    registry.register(msg_type::NOTIF_GET_LIST, Arc::new(|ctx, frame| {
        Box::new(Box::pin(async move {
            match db::get_notifications(&ctx.conn_mgr.db, ctx.user.id).await {
    ...
    registry.register(msg_type::RECORD_INTERACTION, Arc::new(|ctx, frame| {
        Box::new(Box::pin(async move {
            ...
            let weight = data.get("weight").and_then(|v| v.as_i64()).unwrap_or(1) as i32;
            let _ = db::record_interaction(&ctx.conn_mgr.db, ctx.user.id, post_id, interaction_type, weight).await;
            ctx.respond_json(&frame, &serde_json::json!({"ok": true}));
        }))
    })).await;
